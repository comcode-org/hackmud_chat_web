...
if (typeof window != "undefined") {
    ...
    request = function(ops, cb) {
        ...
        x.onreadystatechange = function() {
            ...
            cb(null, {
                statusCode: x.status
            }, x.responseText ? JSON.parse(x.responseText) : []);
        }
...
var API = {
        ...
        // core API
        get_token: (pass) => API.__promise_wrap('get_token', {
            pass: pass
        }),
        account_data: (token) => API.__promise_wrap('account_data', {
            chat_token: token
        }),
        chats: (token, usernames, ext = {}) => API.__promise_wrap('chats', Object.assign(ext, {
            chat_token: token,
            usernames: usernames
        })),
        send: (token, username, channel, msg) => API.__promise_wrap('create_chat', {
            chat_token: token,
            username: username,
            channel: channel,
            msg: msg
        }),
        tell: (token, username, user, msg) => API.__promise_wrap('create_chat', {
            chat_token: token,
            username: username,
            tell: user,
            msg: msg
